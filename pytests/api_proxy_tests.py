...
class APIProxyTest(unittest.TestCase):
    def test_post_record_play_via_proxy(self):
        ...
        resp = requests.post('http://localhost:8081/_proxy', json = data, headers = headers, proxies = proxy_servers, verify = False)
        original = resp.text
        ...
        resp = requests.post('http://localhost:8080/todos', json = data, headers = headers)
    ...
    def test_get_record_play_via_proxy(self):
        ...
        resp = requests.get('http://localhost:8081/_proxy', headers = headers, proxies = proxy_servers, verify = False)
        self.assertEqual(200, resp.status_code)
        headers = {
            'Content-Type': 'application/json',
        }
        resp = requests.get('http://localhost:8080/todos/1', headers = headers)
        todo = json.loads(resp.text)
        self.assertEqual(1, todo['userId'])
    ...
    def test_post_record_play(self):
        ...
        data = {'userId': 1, 'title': 'Buy milk', 'completed': False}
        resp = requests.post('http://localhost:8080/_proxy', json = data, headers = headers)
        original = resp.text
        self.assertEqual(201, resp.status_code)
        todo = json.loads(resp.text)
        self.assertEqual(1, todo['userId'])
    ...
    def test_get_record_play(self):
        ...
        resp = requests.get('http://localhost:8080/_proxy', headers = headers)
        self.assertEqual(200, resp.status_code)
        headers = {
            'Content-Type': 'application/json',
        }
